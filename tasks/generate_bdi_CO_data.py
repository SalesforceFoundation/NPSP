...
def make_records(batch_size, factories):
    """Make the ?XXX? batches of DIs described here:
    ...
    create_batch(
        ...
        npsp__GAU_Allocation_1_GAU__c="Scholarship",
        npsp__Opportunity_Contact_Role_1_Role__c="X",
        npsp__CO1_Text__c=factory.LazyAttribute(lambda o: f"Account {o.counter(0)}"), # # why not in org???
        npsp__CO1_Date__c=date(),
        npsp__CO1_currency__c=100,
        npsp__CO1_Number__c=1,
        npsp__CO1_Picklist__c=factory.SequenceAttribute(lambda i: f"Option{(i%4) + 1}"),
        ...
        npsp__CO2_Currency2__c=200,   # ## CHECK THIS ONE
        npsp__CO3_Text__c=factory.LazyAttribute(lambda o: f"text{o.counter(0)}"),
        npsp__CO3_Date__c=date(),
        npsp__CO3_Currency__c=100,
        npsp__CO3_Number__c=1,
        npsp__CO3_Picklist__c=factory.SequenceAttribute(lambda i: f"Option{(i%4) + 1}"),
        npsp__CO3_Phone__c=123,
        npsp__WO_RecommendedCrewSize__c=10,
        npsp__WO_SuggestedMaintenanceDate__c=date(),
