...
class ObjectManagerPage(BaseNPSPPage, BasePage):
	...
	def open_fields_and_relationships(self, object_name):
		...
		if object_name == "Payment":  # Need this extra logic as Payment is not the same as other standard objects
			...
			leftnavoption = npsp_lex_locators['object_manager']['sidenav_option'].format("Fields & Relationships")
	...
	def create_currency_field(self,field_name):
		""" Creates a custom field of type currency"""
		...
		self.selenium.wait_until_page_contains_element(currency_locator,60)
	...
	def create_formula_field(self,field_name,formula):
		...
		check_syntax = npsp_lex_locators['object_manager']['input_validation']
	...
	def create_custom_field(self, type, field_name, related_to=None, formula=None):
		...
		self.selenium.get_webelement(search_button).send_keys(Keys.ENTER)
		time.sleep(1)
		self.builtin.log_to_console(formula)
		self.salesforce.wait_until_loading_is_complete()
