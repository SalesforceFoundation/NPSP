...
*** Keywords ***
Setup Test Data
    [Documentation]       Setting up contact and account names, dates, namespace required for the test
...
Verify Donation Creation Fails on Incorrect Data and Reprocess
    [Documentation]   Create a DI record with Account, CustomObject2 and Donation details but select
    ...               Donation Donor as Contact.Verify that DI processing fails but account is created.
    ...               Edit DI and change Donor to Account and reprocess DI record.DI completes and
    ...               account matches to previous and Donation and Custom Object records are created
    ...
    Verify Expected Values       nonns    Account
    ...    ${data_import_upd}[${ns}Account1Imported__c]
    ...
    Verify Expected Values           nonns    Opportunity
    ...    ${data_import_upd}[${ns}DonationImported__c]
    ...    Amount=100.0
    ...    CloseDate=${date}
    ...
    Verify Expected Values           nonns    npe01__OppPayment__c
    ...    ${data_import_upd}[${ns}PaymentImported__c]
    ...    npe01__Paid__c=True
    ...
    Verify Expected Values           nonns       CustomObject2__c
    ...    ${data_import_upd}[${org_ns}CustomObject2Imported__c]
...
Verify GAU Allocation Fails on Incorrect Data and Reprocess
    [Documentation]   Create a DI record with Contact, correct Donation details and GAU Allocation percent
    ...               but without GAU ID. Verify that DI processing fails but Contact, Donation and Payment
    ...               are created. Update DI with GAU ID and reprocess DI record. DI completes and Contact
    ...               matches to previous and Donation and GAU allocation records are created
    [tags]                 W-035913    feature:BDI    unstable
    ...
    Verify Expected Values           nonns    Contact
    ...    ${data_import_upd}[${ns}Contact1Imported__c]
