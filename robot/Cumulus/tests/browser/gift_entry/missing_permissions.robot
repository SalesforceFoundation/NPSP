...
Get Field ID
  ...
  ${result} =             Soql Query  SELECT Id,Field,SObjectType,PermissionsRead,PermissionsEdit FROM FieldPermissions WHERE parentId IN ( SELECT id FROM permissionset WHERE PermissionSet.Profile.Name = '${profile_name}') AND SobjectType='${object_name}' AND Field='${field_name}'
...
Setup Test Data
  ...
  ${DI_BATCH_DESCRIPTION_ID} =  Get Field ID  ${profile_name}  Data_Import_Batch__c  Batch_Description__c
